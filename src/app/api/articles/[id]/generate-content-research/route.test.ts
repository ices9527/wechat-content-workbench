...
describe("/api/articles/[id]/generate-content-research route initialization errors", () => {
  beforeEach(() => {
  ...
  it("returns JSON when generation initialization fails", async () => {
    const response = await POST(
      new Request("http://localhost/api/articles/article-1/generate-content-research", { method: "POST" }) as never,
      params()
    );
