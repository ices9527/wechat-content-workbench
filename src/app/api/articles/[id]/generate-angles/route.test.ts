...
describe("/api/articles/[id]/generate-angles route initialization errors", () => {
  beforeEach(() => {
  ...
  it("returns JSON when generation initialization fails", async () => {
    const response = await POST(
      new Request("http://localhost/api/articles/article-1/generate-angles", { method: "POST" }) as never,
      params()
    );
