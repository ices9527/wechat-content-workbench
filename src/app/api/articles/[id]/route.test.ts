...
describe("/api/articles/[id] route initialization errors", () => {
  beforeEach(() => {
  ...
  it("returns JSON when detail initialization fails", async () => {
    const response = await GET(new Request("http://localhost/api/articles/article-1") as never, params());
