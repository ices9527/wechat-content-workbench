...
describe("/api/articles/[id]/accept-outline route initialization errors", () => {
  beforeEach(() => {
  ...
  it("returns JSON when acceptance initialization fails", async () => {
    const response = await POST(
      new Request("http://localhost/api/articles/article-1/accept-outline", { method: "POST" }) as never,
      params()
    );
