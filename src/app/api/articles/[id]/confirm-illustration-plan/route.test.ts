...
describe("/api/articles/[id]/confirm-illustration-plan route initialization errors", () => {
  beforeEach(() => {
  ...
  it("returns JSON when confirmation initialization fails", async () => {
    const response = await POST(
      new Request("http://localhost/api/articles/article-1/confirm-illustration-plan", { method: "POST" }) as never,
      params()
    );
