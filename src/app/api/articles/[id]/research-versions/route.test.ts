...
describe("/api/articles/[id]/research-versions route initialization errors", () => {
  beforeEach(() => {
  ...
  it("returns JSON when list initialization fails", async () => {
    const response = await GET(new Request("http://localhost/api/articles/article-1/research-versions") as never, params());

    await expectInitializationJsonError(response);
  });

  it("returns JSON when manual save initialization fails", async () => {
    const response = await POST(
      new Request("http://localhost/api/articles/article-1/research-versions", { method: "POST" }) as never,
      params()
    );
