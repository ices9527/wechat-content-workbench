...
describe("/api/articles/[id]/mark-final-draft route initialization errors", () => {
  beforeEach(() => {
  ...
  it("returns JSON when final draft initialization fails", async () => {
    const response = await POST(
      new Request("http://localhost/api/articles/article-1/mark-final-draft", { method: "POST" }) as never,
      params()
    );
