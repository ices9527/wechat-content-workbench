...
describe("/api/articles/[id]/select-angle route initialization errors", () => {
  beforeEach(() => {
  ...
  it("returns JSON when selection initialization fails", async () => {
    const response = await POST(
      new Request("http://localhost/api/articles/article-1/select-angle", { method: "POST" }) as never,
      params()
    );
