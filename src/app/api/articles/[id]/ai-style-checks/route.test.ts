...
describe("/api/articles/[id]/ai-style-checks route initialization errors", () => {
  beforeEach(() => {
  ...
  it("returns JSON when list initialization fails", async () => {
    const response = await GET(new Request("http://localhost/api/articles/article-1/ai-style-checks") as never, params());
