...
describe("/api/articles/[id]/pre-publish-check route initialization errors", () => {
  beforeEach(() => {
  ...
  it("returns JSON when check initialization fails", async () => {
    const response = await POST(
      new Request("http://localhost/api/articles/article-1/pre-publish-check", { method: "POST" }) as never,
      params()
    );
