...
describe("/api/articles/[id]/revise-from-ai-style-check route initialization errors", () => {
  beforeEach(() => {
  ...
  it("returns JSON when AI style check revision initialization fails", async () => {
    const response = await POST(
      new Request("http://localhost/api/articles/article-1/revise-from-ai-style-check", { method: "POST" }) as never,
      params()
    );
