...
describe("/api/articles/[id]/run-publish-html-ai-style-check route initialization errors", () => {
  beforeEach(() => {
  ...
  it("returns JSON when run initialization fails", async () => {
    const response = await POST(
      new Request("http://localhost/api/articles/article-1/run-publish-html-ai-style-check", { method: "POST" }) as never,
      params()
    );
