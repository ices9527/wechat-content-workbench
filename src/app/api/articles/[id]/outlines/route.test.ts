...
describe("/api/articles/[id]/outlines route initialization errors", () => {
  beforeEach(() => {
  ...
  it("returns JSON when list initialization fails", async () => {
    const response = await GET(new Request("http://localhost/api/articles/article-1/outlines") as never, params());
  ...
  it("returns JSON when create initialization fails", async () => {
    const response = await POST(
      new Request("http://localhost/api/articles/article-1/outlines", { method: "POST" }) as never,
      params()
    );
  ...
  it("returns JSON when update initialization fails", async () => {
    const response = await PATCH(
      new Request("http://localhost/api/articles/article-1/outlines", { method: "PATCH" }) as never,
      params()
    );
