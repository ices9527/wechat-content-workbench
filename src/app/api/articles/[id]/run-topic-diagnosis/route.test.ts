...
describe("/api/articles/[id]/run-topic-diagnosis route initialization errors", () => {
  beforeEach(() => {
  ...
  it("returns JSON when run initialization fails", async () => {
    const response = await POST(
      new Request("http://localhost/api/articles/article-1/run-topic-diagnosis", { method: "POST" }) as never,
      params()
    );
