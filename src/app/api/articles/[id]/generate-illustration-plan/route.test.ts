...
describe("/api/articles/[id]/generate-illustration-plan route initialization errors", () => {
  beforeEach(() => {
  ...
  it("returns JSON when generation initialization fails", async () => {
    const response = await POST(
      new Request("http://localhost/api/articles/article-1/generate-illustration-plan", { method: "POST" }) as never,
      params()
    );
