...
describe("/api/articles/[id]/mark-ready-to-publish route initialization errors", () => {
  beforeEach(() => {
  ...
  it("returns JSON when ready-to-publish initialization fails", async () => {
    const response = await POST(
      new Request("http://localhost/api/articles/article-1/mark-ready-to-publish", { method: "POST" }) as never,
      params()
    );
