...
describe("/api/articles/[id]/drafts route initialization errors", () => {
  beforeEach(() => {
  ...
  it("returns JSON when list initialization fails", async () => {
    const response = await GET(new Request("http://localhost/api/articles/article-1/drafts") as never, params());
  ...
  it("returns JSON when create initialization fails", async () => {
    const response = await POST(
      new Request("http://localhost/api/articles/article-1/drafts", { method: "POST" }) as never,
      params()
    );
  ...
  it("returns JSON when update initialization fails", async () => {
    const response = await PATCH(
      new Request("http://localhost/api/articles/article-1/drafts", { method: "PATCH" }) as never,
      params()
    );
